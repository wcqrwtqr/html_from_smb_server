...
input_file="pdf_list_soc_approval.txt"
output_file="output_soc_approval.html"
...
# Open the output file for writing
{
  ...
  while IFS=, read -r pdf_name pdf_path; do
    ...
    if [[ "$pdf_name" = *"Expired"* ]]; then
      continue # Skip this line if "EXPIRED" is not in pdf_name
    fi
    ...
    soc_no=$(echo "$pdf_name" | awk '{print $3}')
