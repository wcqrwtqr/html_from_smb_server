...
input_file="pdf_list_roo_sop.txt"
output_file="output_roo_sop.html"

if [[ ! -f "$input_file" ]] ; then
    echo -e "The file ${RED}pdf_list_roo_sop.txt generated from ./01_get_paths_roo_sop.sh${NC} was not ran"
    exit 1
fi
...
# Open the output file for writing
{
  ...
  while IFS=: read -r pdf_name pdf_path; do
    ...
    rev_no=$(echo "$pdf_name" | grep -o "A\d")
