...

{
    ...
    tail -n +2 "$input_file" | while IFS=',' read -r id sn description type size \
        ...
        mpi_sn=$(awk -F, -v sn="$sn" '$0 ~ "MPI.*" sn {print $2; exit}' "${asset_list}" | sed 's|/Volumes/My Passport for Mac/NEOS/Server Backup/WL-SL|..|g' | tr -d '"')
        if [[ -n $mpi_sn ]]; then
        ...
        utm_sn=$(awk -F, -v sn="$sn" '$0 ~ "UTM.*" sn {print $2; exit}' "${asset_list}" | sed 's|/Volumes/My Passport for Mac/NEOS/Server Backup/WL-SL|..|g' | tr -d '"')
        if [[ -n $utm_sn ]]; then
        ...
        # Search for PT links
        pt_sn=$(awk -F, -v sn="$sn" '$0 ~ "PT.*" sn {print $2; exit}' "${asset_list}" | sed 's|/Volumes/My Passport for Mac/NEOS/Server Backup/WL-SL|..|g' | tr -d '"')
        if [[ -n $pt_sn ]]; then
        ...
        cal_sn=$(awk -F, -v sn="$sn" '$0 ~ "Calib.*" sn {print $2; exit}' "${asset_list}" | sed 's|/Volumes/My Passport for Mac/NEOS/Server Backup/WL-SL|..|g' | tr -d '"')
        if [[ -n $cal_sn ]]; then
        ...
        coc_sn=$(awk -F, -v sn="$sn" '$1 ~ sn "ICC1" {print $2; exit}' "${coc_list}" | sed 's|/Volumes/My Passport for Mac/NEOS/Server Backup/WL-SL|..|g' | tr -d '"')
        if [[ -n $coc_sn ]]; then
