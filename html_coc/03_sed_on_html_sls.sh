...
if [ ! -f "./output_coc.html" ]; then
    echo -e "${RED}Error: Input file 'output_coc.html' not found.${NC}" >&2
    exit 1
fi

# Attempt to run the sed command
if ! sed -i '' 's|/Volumes/My Passport for Mac/NEOS/Server Backup/WL-SL|..|g' ./output_coc.html; then
...
echo -e "${CYAN}File 'output_coc.html' processed successfully.${NC}"
